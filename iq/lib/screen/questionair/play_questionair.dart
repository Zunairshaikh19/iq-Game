...
class PlayQuestionair extends StatefulWidget {
  const PlayQuestionair({super.key, required this.questionair});
  final Questionair questionair;

  @override
  State<PlayQuestionair> createState() => _PlayQuestionairState();
}

class _PlayQuestionairState extends State<PlayQuestionair> {
  bool clicked = false;
  ...
  setQuestion() {
    question = widget.questionair.questions![selected];
    setClicked();
  ...
  next() {
    if (selected == (widget.questionair.questions!.length - 1)) {
      Get.back();
    } else {
      if (selected != (widget.questionair.questions!.length - 1)) {
        selected = selected + 1;
